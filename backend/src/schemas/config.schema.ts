...
function sectionWithLegacyMerge<T extends Record<string, unknown>>(defaults: T, schema: z.ZodType<T>) {
    return z
        .unknown()
        .optional()
        .transform((raw): T => {
...
export const KycConfigSchema = z.object({
    ...
    verificationSteps: sectionWithLegacyMerge(
        { ...DEFAULT_VERIFICATION_STEPS },
        VerificationStepsSchema
    ),
